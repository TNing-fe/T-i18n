...
export const App = () => {
  ...
  const changeProp = (todoindex: number, value: any, prop: any) => {
    updateTodoList(
      todoList.map((item: TodoListItem, _index) => {
        if (prop && _index === todoindex) {
          item[prop] = value;
        }
        return item;
      })
  ...
  const translateOne = (index: any, todoIndex: number) => {
    ...
    vscode.postMessage<TranlateOneMessage>({
      type: 'TranlateOne',
      payload: { needTranslate, to: todoList[index].optionValue, todoindex: index },
    });
  };
  const onSave = () => {
    setLog(JSON.stringify(todoList));
  ...
  return (
    <div>
      ...
      <VSCodeTextField
        value={text}
        onInput={e => {
          const { value } = (e?.target || {}) as Record<string, any>;
          setText(value);
        }}
        style={{ width: '400px' }}
      >
      ...
      {todoList.map((item, todoindex) => (
        <div>
          <VSCodeTextField
            placeholder="翻译结果(添加的键值)"
            value={item.targetValue}
            onChange={e => translationResChange(e, todoindex)}
          ></VSCodeTextField>
          <VSCodeDropdown disabled={isOnSave} value={item?.optionValue || '选择语言'}>
            {checkLan.map((item, index) => (
              <VSCodeOption
                key={item.type}
                value={item.type}
                onClick={() => changeProp(todoindex, item.type, 'optionValue')}
              >
                {item.name}
              </VSCodeOption>
            ))}
          ...
          <VSCodeDropdown disabled={isOnSave} value={item?.fileType || '选择文件类型'}>
            <VSCodeOption value="" onClick={() => changeProp(todoindex, 'object', 'fileType')}>
              选择文件类型
            </VSCodeOption>
            <VSCodeOption
              value="object"
          ...
          <VSCodeTextField
            disabled={isOnSave}
            value={item.filePath}
            placeholder="目标文件绝对路径"
            type="text"
            onChange={e => {
              const { value } = (e?.target || {}) as Record<string, any>;
              changeProp(todoindex, value, 'filePath');
            }}
            id={`file${todoindex}`}
          ...
          <VSCodeButton
            style={{ position: 'relative', bottom: '12px' }}
            onClick={() => translateOne(todoindex, todoindex)}
          >
            翻译
          </VSCodeButton>
          <VSCodeButton
            style={{ position: 'relative', bottom: '12px' }}
            onClick={() => delOne(todoindex)}
          >
            删除
          </VSCodeButton>
        </div>
